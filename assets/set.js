...
async function boot(){
  ...
  /* ชุดที่เปิดโหมดกระดาษ: ให้เลือกโหมดก่อน
     ถ้ายังไม่ได้เริ่มจับเวลา = กลับมาให้เลือกโหมดใหม่ได้ (เผื่อเปลี่ยนใจ/หลงกดเข้ามา)
     แต่ถ้าเริ่มจับเวลาไปแล้ว ต้องกลับไปที่เดิม ห้ามเด้งออก */
  if(META && META.paper && MODE !== 'drill'){
    paper = paper || newPaper();
    if(!MODE || !paper.startedAt) return renderModePick();
    return renderPaper();
  }
  MODE = MODE || 'drill';
...
const listUrl = () => 'set.html' + (ROOM !== 'posn' ? '?room=' + ROOM : '');

function renderModePick(){
  const P = paperScreen();
  P.innerHTML =
      '<div class="pcard">'
    +   '<button class="pback" id="pBackList">ย้อนกลับ</button>'
    ...
    + '</div>';
  $('pBackList').onclick = () => { location.href = listUrl(); };
...
function renderPaper(){
  ...
  if(!started){
    h = '<div class="pcard">'
      +   '<button class="pback" id="pBackMode">ย้อนกลับ</button>'
  ...
  if($('pBackClock')) $('pBackClock').onclick = undoDone;
  if($('pBackList2')) $('pBackList2').onclick = () => { location.href = listUrl(); };
  if(done) bindSheet(P);
...
function renderSheet(){
  ...
  h += graded
    ? '<button class="pback" id="pBackList2">ย้อนกลับ</button>'
    : '<button class="pback" id="pBackClock">ย้อนกลับ • ยังทำไม่เสร็จ</button>';
  if(graded){
...
/* กดทำเสร็จแล้วแต่ยังไม่เสร็จจริง → กลับไปจับเวลาต่อ
   ช่วงที่มานั่งอยู่หน้าติ๊กคำตอบ นับเป็นเวลาพัก ไม่บวกเข้าเวลาทำ */
function undoDone(){
  if(!paper.doneAt || paper.gradedAt) return;
  paper.pausedMs += Date.now() - paper.doneAt;
  paper.doneAt = null; pendingGrade = false;
  save(); renderPaper(); window.scrollTo(0, 0);
}
